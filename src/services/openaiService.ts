...
export const getMagicPrompt = async (name: string) => {
    try {
      const response = await fetch(
        ...
        {
          headers: { Authorization: "Bearer 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" },
          method: "POST",
...
//-------------------------------------------------------
//  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
...
export const createImage = async (apiUrl: string, prompt: string) => {
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      throw new Error(`API Error (${error.response.status}): ${error.response.data.toString()}`);
